...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type == 'rnn':
            ...
            self.feature_proj = nn.Linear(self.encoder.out_channels, hidden_dim)
        elif cell_type == 'lstm':
    ...
    def forward(self, images, captions):
        ...
        feats = self.encoder(images)  # (N, C, 4, 4)

        if self.cell_type == 'attn':
            N, C, h, w = feats.shape
            A = self.feature_proj(feats.permute(0, 2, 3, 1))  # (N, 4, 4, H)
            ...
            h0 = self.feature_proj(feats_pooled)
            word_vecs = self.word_embed(captions_in)
            hidden = self.rnn(word_vecs, h0)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == 'attn':
            N, C, h, w = feats.shape
            A = self.feature_proj(feats.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            prev_h = A.mean(dim=(2, 3))
            prev_c = prev_h.clone()
        elif self.cell_type == 'lstm':
            prev_h = self.feature_proj(feats.mean(dim=(2, 3)))
...
def dot_product_attention(prev_h, A):
    ...
    q = prev_h.unsqueeze(1)                      # (N, 1, H)
    scores = torch.bmm(q, A_flat) / (H ** 0.5)   # (N, 1, 16)
    attn_weights = F.softmax(scores, dim=2).squeeze(1)  # (N, 16)
    attn = torch.bmm(attn_weights.unsqueeze(1), A_flat.permute(0, 2, 1)).squeeze(1)  # (N, H)
