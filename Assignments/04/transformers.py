...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.MultiHeadBlock = self.attention
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = pos.unsqueeze(1).expand(K, M).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for i in range(0, M, 2):
        div = torch.pow(torch.tensor(10000.0), torch.tensor(i / M))
        y[:, i] = torch.sin(pos[:, 0] / div)
        if i + 1 < M:
            y[:, i + 1] = torch.cos(pos[:, 0] / div)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
